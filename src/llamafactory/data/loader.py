...
def _get_merged_dataset(
    ...
    for dataset_name, dataset_attr in zip(dataset_names, get_dataset_list(dataset_names, data_args.dataset_dir)):
        ...
        datasets[dataset_name] = _load_single_dataset(dataset_attr, model_args, data_args, training_args, streaming=streaming)
...
def get_dataset(
    ...
    if data_args.tokenized_path is not None:
        if has_tokenized_data(data_args.tokenized_path):
            ...
            if data_args.eval_streaming and dataset_module.get("eval_dataset") is not None:
                if isinstance(dataset_module["eval_dataset"], dict):
                    dataset_module["eval_dataset"] = {
                        k: v.to_iterable_dataset() for k, v in dataset_module["eval_dataset"].items()
                    }
                else:
                    dataset_module["eval_dataset"] = dataset_module["eval_dataset"].to_iterable_dataset()


            logger.info_rank0(f"Loaded tokenized dataset from {data_args.tokenized_path}.")
            return dataset_module

        if data_args.streaming or data_args.eval_streaming:
            raise ValueError("Turn off `streaming`/`eval_streaming` when saving dataset to disk.")

    # Load and preprocess dataset
    with training_args.main_process_first(desc="load dataset", local=(not data_args.data_shared_file_system)):
        dataset = _get_merged_dataset(
            data_args.dataset, model_args, data_args, training_args, stage, streaming=data_args.streaming
        )
    ...
    with training_args.main_process_first(desc="pre-process dataset", local=(not data_args.data_shared_file_system)):
        ...
        if "train" in train_dict:
            train_dict["train"] = _get_preprocessed_dataset(
                train_dict["train"], data_args, training_args, stage, template, tokenizer, processor, streaming=data_args.streaming, is_eval=False
            )

        for key in eval_dict:
            eval_dict[key] = _get_preprocessed_dataset(
                eval_dict[key], data_args, training_args, stage, template, tokenizer, processor, streaming=data_args.eval_streaming, is_eval=True
            )
